...
function renderBeatStrip(activeBeat = 1) {
  ...
  beatStrip.style.setProperty("--beats", world.meter.numerator);
  beatStrip.innerHTML = "";
  for (let i = 1; i <= world.meter.numerator; i += 1) {
    const dot = document.createElement("span");
    dot.className = `beat-dot${i === 1 ? " downbeat" : ""}${i === activeBeat ? " active" : ""}`;
...
function updateRhythmLabels(activeBeat = 1) {
  if (tempoLabel) {
    tempoLabel.textContent = `${world.bpm} BPM`;
  }
  if (meterLabel) {
    const tableBeat = activeBeat + getSubdivisionMode().fraction;
    const countInText = world.countInBars > 0 ? ` · count ${world.countInBars} bar` : "";
    meterLabel.textContent = `${world.meter.numerator}/${world.meter.denominator} · ${getSubdivisionMode().label} · hit ${activeBeat}, table ${tableBeat.toFixed(2).replace(/0$/, "")}${countInText}`;
  }
  if (beatLabel) {
    beatLabel.textContent = `${activeBeat}`;
  }
...
function updateTransportButtons() {
  if (playPauseButton) {
    playPauseButton.textContent = world.isPlaying ? "Pause" : "Start";
  }
}
...
function drawServeBall(sample) {
  const ballRadius = getBallRadius();
  const shadowRadius = ballRadius * mix(1.15, 2.8, clamp(sample.height / 1.8, 0, 1));
  ctx.save();
  ctx.fillStyle = `rgba(12, 16, 15, ${mix(0.24, 0.045, clamp(sample.height / 1.8, 0, 1))})`;
  ctx.beginPath();
  ctx.ellipse(
    sample.projection.x,
    sample.projection.y,
...
function draw(timestamp) {
  ...
  if (!world.current && clockMs < world.countInEnd) {
    ...
    world.lastTime = clockMs;
    stateLabel.textContent = "count-in";
  ...
  if (beatLabel) {
    ...
    beatLabel.textContent = isTableBeat ? tableBeat.toFixed(2).replace(/0$/, "") : `${shot.beatNumber}`;
  }
