...
let countInPreviewActive = false;
...
const metronome = window.AmbientMetronome.createMetronome({
  ...
  onBeat: (state, accent) => {
    if (countInPreviewActive && !state.countingIn) {
      countInPreviewActive = false;
      const now = performance.now();
      beginPointTransition(now);
      beginSplitTransition(now, now / 1000 / 4);
    }
...
function updateCameraControls() {
  ...
  if (cameraSwitchButton) {
    cameraSwitchButton.textContent = cameraFacingMode === "user" ? "切到后置" : "切到前置";
    cameraSwitchButton.disabled = !navigator.mediaDevices?.getUserMedia;
  }
}
...
async function startCamera(facingMode = cameraFacingMode) {
  ...
  try {
    ...
    if (actualFacing === "user" || actualFacing === "environment") cameraFacingMode = actualFacing;
    setCameraStatus(cameraFacingMode === "environment" ? "后置摄像头已开启" : "前置摄像头已开启");
  } catch (error) {
    stopCamera();
    setCameraStatus(error?.name === "NotAllowedError" ? "摄像头权限未开启" : "摄像头启动失败");
  }
...
function motionPhase(time, now) {
  ...
  if (!rhythm.running || rhythm.countingIn) {
    return time * speed;
  }
...
function beginSplitTransition(now, fromClock = splitMorphClock(now)) {
  splitTransition = {
    startAt: now,
    duration: 260,
    clock: fromClock,
...
function splitMorphOffset(key, axis, span, usable, now) {
  const previewDamping = rhythm.running && !rhythm.countingIn ? 1 : 0.35;
...
function updatePoints(time, now) {
  ...
  const visualBeatRunning = rhythm.running && !rhythm.countingIn;
  const pulseAmp = visualBeatRunning ? 1 + beatEnergy * (rhythm.beatIndex === 0 ? 1.45 : 0.95) : 1;
...
cameraToggleButton?.addEventListener("click", () => {
  ...
  startCamera(cameraFacingMode).then(() => {
    showRhythmPanel(rhythm.running ? 2200 : 0);
  });
});
...
cameraSwitchButton?.addEventListener("click", () => {
  ...
  updateCameraControls();
  if (cameraEnabled) {
    startCamera(nextFacingMode).then(() => {
      showRhythmPanel(rhythm.running ? 2200 : 0);
    });
  } else {
    setCameraStatus(nextFacingMode === "environment" ? "将使用后置摄像头" : "将使用前置摄像头");
  }
});
...
playPauseButton.addEventListener("click", async () => {
  ...
  if (rhythm.running) {
    ...
    const willPreviewCountIn = rhythm.countInBars > 0;
    if (willPreviewCountIn) {
      countInPreviewActive = true;
      pointTransition = null;
      splitTransition = null;
    } else {
      countInPreviewActive = false;
